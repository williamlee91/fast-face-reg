...
import sys
sys.path.append("/home/yww/hdf5dataset")
...
from keras.layers import GlobalAveragePooling2D, SeparableConv2D,DepthwiseConv2D
...
class Model_train:
    ...
    def conv_block(self, inputs, filters, block_id, kernel=(3, 3), strides=(1, 1)):
        """
        Normal convolution block performs conv+bn+elu operations.
    ...
    def fire_squeeze(self, inputs, out_channels, block_id):
        """
        Normal fire_squeeze block performs conv+bn+elu operations.
        :param inputs: Input Keras tensor in (B, H, W, C_in)
    ...
    def dw_conv(self, x, stride, weight_decay, block_id):
        """
        This function defines a sequence of 1 or more identical layers, referring to Table 2 in the original paper.
        :param x: Input Keras tensor in (B, H, W, C_in)
        :param stride: stride for the 1x1 convolution
    ...
    def Squeezefacenet(self):
        ...
        #stage 0
        x0 = self.conv_block(input_img, 96, 1)
        x0 = self.dw_conv(x0, 2, weight_decay, 0)
        
        #stage 1
        x1 = self.fire_squeeze(x0, 32, 1) 
        x1 = self.dw_conv(x1, 2, weight_decay, 1)
        ...
        x4 = self.dw_conv(x4, 2, weight_decay, 4)
        ...
        x10 = self.dw_conv(x10, 2, weight_decay, 10) 
    ...
    def train(self, data_path):  
        ...
        optimizer = Adam(lr=0.02, beta_1= 0.9, beta_2= 0.999, epsilon= 0.1, decay=1e-5)
        ...
        plt.savefig('model9_1_new/acc9_bigdata.png')
        plt.close()
        
        plt.plot(epochs, loss, 'r-')
        plt.plot(epochs, val_loss, 'b-')
        plt.title('Training and validation loss')
        plt.savefig('model9_1_new/loss9_bigdata.png') 
...
class EveryEpochCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):  #log must be defined
        self.model_to_save.save('model9_1_new/model_%d.h5' % epoch)    
...
class HignestAccCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None): #log must be defined
        ...
        if acc >= self.highest: # save best model
            self.highest = acc
            self.model_to_save.save('model9_1_new/best_model.h5')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('model9_1_new'):
        os.makedirs('model9_1_new')
    ...
    print('Model Saved.')
    model.save_model(file_path = 'model9_1_new/best_weights.h5')

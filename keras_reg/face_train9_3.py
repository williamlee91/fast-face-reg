...
class Model_train:
    ...
    def fire_squeeze(self, inputs, out_channels, block_id):
        ...
        fire_expand2 =SeparableConv2D(out_channels * 4, 
                              (3, 3),
                              padding='same',
                              strides=(1, 1), 
                              depth_multiplier=1,
    ...
    def train(self, data_path):  
        ...
        plt.savefig('model9_3/acc9_bigdata.png')
        plt.close()
        
        plt.plot(epochs, loss, 'r-')
        plt.plot(epochs, val_loss, 'b-')
        plt.title('Training and validation loss')
        plt.savefig('model9_3/loss9_bigdata.png') 
...
class EveryEpochCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):  #log must be defined
        self.model_to_save.save('model9_3/model_%d.h5' % epoch)    
...
class HignestAccCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None): #log must be defined
        ...
        if acc >= self.highest: # save best model
            self.highest = acc
            self.model_to_save.save('model9_3/best_model.h5')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('model9_3'):
        os.makedirs('model9_3')
    ...
    print('Model Saved.')
    model.save_model(file_path = 'model9_3/best_weights.h5')

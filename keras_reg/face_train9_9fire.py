...
class Model_train:
    ...
    def train(self, data_path):  
        ...
        plt.savefig('model9_9fire/acc9_bigdata.png')
        plt.close()
        
        plt.plot(epochs, loss, 'r-')
        plt.plot(epochs, val_loss, 'b-')
        plt.title('Training and validation loss')
        plt.savefig('model9_9fire/loss9_bigdata.png') 
...
class EveryEpochCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):  #log must be defined
        self.model_to_save.save('model9_9fire/model_%d.h5' % epoch)    
...
class HignestAccCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None): #log must be defined
        ...
        if acc >= self.highest: # save best model
            self.highest = acc
            self.model_to_save.save('model9_9fire/best_model.h5')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('model9_9fire'):
        os.makedirs('model9_9fire')
    ...
    print('Model Saved.')
    model.save_model(file_path = 'model9_9fire/best_weights8_big_96.h5')

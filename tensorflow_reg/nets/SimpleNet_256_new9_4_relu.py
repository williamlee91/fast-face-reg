...
def csnet(images, bottleneck_layer_size, is_training):
    ...
    #stage 1
    x1 = csf_random(x0, 24, 32, "fire1")  
    x1 = ddw_conv(x1, depth=80, scope="dw_conv1")
    
    #stage 2
    x2 = csf_random(x1, 40, 32, "fire2")  
    
    #stage 3
    x3 = csf_random(x2, 32, 48, "fire3")  
    x3 = ddw_conv(x3, depth=128, scope="dw_conv3")
    
    #stage 4
    x4 = csf_random(x3, 64, 56,"fire4") 
       
    #stage 5
    x5 = csf_random(x4, 56, 80, "fire5")               
    x5 = ddw_conv(x5, depth=192, scope="dw_conv5")
    ...
    #augment
    #x7 = slim.conv2d(x7, 512, [1,1], stride=1, padding="SAME", scope="conv7")
...
def fire(input_tensor, squeeze_depth, scope=None):
    ...
    with tf.variable_scope(scope):
        ...
        expand_3x3 = slim.separable_conv2d(inputs=squeeze, 
                                      ...
                                      depth_multiplier=1.0,
                                      scope = "dwconv_3x3")
        return tf.add(expand_1x1, expand_3x3, name="add1")
...
def csf(input_tensor, depth, scope=None):
    ...
    """
    with tf.variable_scope(scope):
        net_left, net_right = tf.split(input_tensor, [int(depth), int(depth)], axis=3, name="split")
...
def csf_half(input_tensor, depth, scope=None):
    ...
    with tf.variable_scope(scope):
        ...
        conv_1x1 = slim.conv2d(concate, (2*depth), [1, 1],stride=1, 
                               padding="SAME",scope="conv_1x1")
        return conv_1x1  

def csf_random(input_tensor, in_depth, out_depth, scope=None):
    """
    channels split fire module with the transpose of left and right parts
    :param input_tensor: Input tensor in (N, H, W, in_channels)
    :param depth: channels for the depthwise convolution
    :param stride: stride for downsampling
    :return: Output tensor (B, H_new, W_new, 2*out_depth)
    """
    with tf.variable_scope(scope):
        net_left, net_right = tf.split(input_tensor, [int(in_depth), int(in_depth)], axis=3, name="split")
        
        conv_3x3 = slim.conv2d(net_right, in_depth, [3,3], stride=1,
                             padding="SAME", scope="conv_3x3")
         
        concate = tf.concat([net_left, conv_3x3], axis=3, name="concat")
        
        conv_1x1 = slim.conv2d(concate, (2*out_depth), [1, 1],
                               stride=1, padding="SAME", scope="conv_1x1")
        return conv_1x1 

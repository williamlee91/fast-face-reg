...
from utils.AMSGrad_layer_rotation import AMSGrad_v2
...
def train(total_loss, global_step, optimizer, learning_rate, moving_average_decay, update_gradient_vars, summaries,
    ...
    with tf.control_dependencies([loss_averages_op]):
        if optimizer == 'ADAGRAD':
            ...
            opt = tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.99, epsilon=0.1)
        elif optimizer == 'RMSPROP':
            opt = tf.train.RMSPropOptimizer(learning_rate, decay=0.9, momentum=0.9, epsilon=1.0)
        elif optimizer == 'MOM':
            opt = tf.train.MomentumOptimizer(learning_rate, 0.9, use_nesterov=True)
        elif optimizer == 'AMSGRAD':
            opt = AMSGrad(learning_rate, beta1=0.9, beta2=0.99, epsilon=0.1) 
        elif optimizer == 'AMSGRAD_V2':
            opt = AMSGrad_v2(learning_rate, beta1=0.9, beta2=0.99, epsilon=0.1) 
        else:
